...
function Logo() {
  return (
    <Link href="/" className="flex items-center gap-2.5 select-none shrink-0">
      <span className="flex h-8 w-8 items-center justify-center rounded-lg bg-green-600 text-white font-black text-sm shadow-sm">
        TL
      </span>
      <span className="hidden sm:block font-extrabold tracking-tight text-gray-900 dark:text-white text-lg">
        Template<span className="text-green-600">Lab</span>
      </span>
    </Link>
  );
}
  ...
  return (
    <div ref={containerRef} className="relative w-full">
      ...
      <input
        ...
        className="w-full rounded-full border border-gray-200 bg-white py-2 pl-8 pr-7 text-sm outline-none transition focus:border-green-500 focus:ring-2 focus:ring-green-100 dark:border-gray-700 dark:bg-gray-800 dark:text-white dark:placeholder:text-gray-500"
      />
      {value && (
        <button
          ...
          {results.length === 0 ? (
            ...
            <>
              ...
              <button
                ...
                className="w-full border-t border-gray-100 px-4 py-2 text-center text-xs text-green-600 transition hover:bg-green-50 dark:border-gray-700 dark:hover:bg-green-900/20"
              >
                Xem tất cả kết quả →
              </button>
            </>
  ...
  return (
    <div ref={ref} className="relative shrink-0">
      ...
      <button
        onClick={() => setOpen((o) => !o)}
        title="Cài đặt"
        className="flex h-9 w-9 items-center justify-center rounded-full border border-gray-200 bg-white text-base shadow-sm transition hover:bg-gray-50 dark:border-gray-700 dark:bg-gray-800 dark:hover:bg-gray-700"
        aria-expanded={open}
      >
        ⚙️
      </button>
                ...
                {(["light", "dark", "system"] as Theme[]).map((t) => (
                  <button
                    ...
                    className={`flex w-full items-center gap-2.5 rounded-xl px-3 py-2.5 text-sm transition ${
                      theme === t
                        ? "bg-green-50 font-semibold text-green-600 dark:bg-green-900/20"
                        : "text-gray-700 hover:bg-gray-50 dark:text-gray-300 dark:hover:bg-gray-700"
                    }`}
                  >
                    <span>{THEME_ICONS[t]}</span>
                    <span>{THEME_LABELS[t]}</span>
                    {theme === t && <span className="ml-auto text-green-500">✓</span>}
                  </button>
                ))}
              </div>
            )}
