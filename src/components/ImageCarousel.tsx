    ...
    <>
      <div className="overflow-hidden rounded-xl border-2 border-gray-200 dark:border-gray-700 shadow-md"
        ...
        <div className="flex items-center gap-1.5 border-b border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 px-3 py-2">
          ...
          <span className="ml-2 flex-1 rounded-xl bg-gray-100 dark:bg-gray-700 px-3 py-1 text-xs text-gray-400 truncate">
        ...
        {/* Slide area */}
        <div className="relative aspect-video w-full bg-gray-100 dark:bg-gray-700 cursor-zoom-in group"
        ...
        {/* Controls */}
        {images.length > 1 && (
          <div className="flex items-center justify-between bg-white dark:bg-gray-800 px-4 py-2">
            {/* Prev/Next */}
            <div className="flex gap-1">
              <button onClick={prev} className="rounded-xl p-1.5 text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-700 hover:text-gray-700 dark:hover:text-gray-200 transition-colors">
                <svg className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                  <path strokeLinecap="round" strokeLinejoin="round" d="M15 19l-7-7 7-7" />
                </svg>
              </button>
              <button onClick={next} className="rounded-xl p-1.5 text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-700 hover:text-gray-700 dark:hover:text-gray-200 transition-colors">
                <svg className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
            ...
            <div className="flex items-center gap-1.5">
              {images.map((_, i) => (
                <button key={i} onClick={() => setCurrent(i)}
                  className={`rounded-full transition-all ${i === current ? "h-2 w-5 bg-green-500" : "h-2 w-2 bg-gray-300 dark:bg-gray-600 hover:bg-gray-400 dark:hover:bg-gray-400"}`}
                />
              ))}
            </div>
            {/* Thumbnail strip */}
